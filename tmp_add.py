...
# The exact data and functional components to insert
TO_INSERT = """
const SERVICES_DATA = {
    creator: {
        theme: "text-[#FF9933]",
...
with open('c:/AOVASITE2/aova/src/app/page.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Rename existing `InteractiveServices` to `PricingServices`
content = content.replace("function InteractiveServices() {", "function PricingServices() {")

# 2. Rename the tag in HomePage from <InteractiveServices /> to <PricingServices />
# Wait, let's just make HomePage have BOTH:
content = content.replace("<InteractiveServices />", "<InteractiveServices />\n            <PricingServices />")

# 3. Add original InteractiveServices just before PricingServices (which used to be named InteractiveServices)
content = content.replace("function PricingServices() {", TO_INSERT + "\n\nfunction PricingServices() {")

with open('c:/AOVASITE2/aova/src/app/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
